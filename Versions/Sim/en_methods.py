...
class Blob:
    ...
    def __init__(self, x=None, y=None, energy=None, phago=None, phyto=None, speed=None, vision=None,
    ...
    def fight(self, blob:'Blob')->None:
        ...
        if self.energy>0 and blob.energy>0 and self!=blob and abs( blob.phago - self.phago ) > 0.1:
            ...
            if blob.phago > self.phago:
                blob.energy += self.energy * 0.25 * blob.phago #OJO; parámetro provision cambiar también abajo
                self.energy = -100
                        
            
            else:       # self eats blob 
                self.energy += blob.energy * 0.25 * self.phago #mismo parámetro: cambiar también
                blob.energy = -100
    ...
    def vital(self, grid:'Grid', metabo:float = 0.1, phytoGain:float = 1.0)->None:
        ...
        if random.random() < self.phyto/(len(grid.blobs_at_tile(self.x, self.y))+0.3*len(grid.get_neighbours_dist(self.x, self.y, 1)) ):
            self.energy += phytoGain*(1-self.phago)
    ...
    def is_alive(self, death_cause_list:list[int])->bool: 
    ...
    def reproduce(self, giving_birth_cost:float=1.2, geneticVar:float=0.01)->list['Blob']:
        ...
        for _ in range(babies_num):
            babies.append(Blob(self.x+random.randint(-1, 1), self.y+random.randint(-1, 1), 
                                energy= babies_energy/babies_num, 
